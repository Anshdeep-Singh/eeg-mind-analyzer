...
export const generateMedicalReportPDF = (data: ClinicalReportData): void => {
  ...
  const drawHeader = (customTitle?: string) => {
    ...
    doc.text('EEG Mind Analyzer | Session Report', margin, 17);
  ...
  const drawFooter = (pageNum: number, totalPages: number) => {
    ...
    const modeLabel = isAi
      ? `Live AI Model (${data.auditOutput?.providerUsed.toUpperCase()})`
      : 'Rule-Based Deterministic Engine (Offline / Non-AI)';
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Assessment Mode:', margin + 4, y + 23.5);
  doc.setFont('helvetica', 'normal');
  const isAiReport = data.auditOutput?.isAiGenerated ?? false;
  if (isAiReport) {
    doc.setTextColor(16, 185, 129);
    doc.text(`Live AI Model (${data.auditOutput?.providerUsed.toUpperCase()} - ${data.auditOutput?.modelUsed})`, margin + 38, y + 23.5);
  } else {
    doc.setTextColor(217, 119, 6);
    doc.text('Rule-Based Engine (Offline / No API Key Provided)', margin + 38, y + 23.5);
  }
  doc.setTextColor(darkTextColor[0], darkTextColor[1], darkTextColor[2]);
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
  doc.text('4. AI Clinical Assessment', margin, y);
  y += 5;
...
export const generateComparativeReportPDF = (data: DualSessionReportData): void => {
  ...
  const drawHeader = () => {
    ...
    doc.text('EEG Mind Analyzer | Comparative Analysis', margin, 17);
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
  doc.text('3. AI Comparative Assessment', margin, y);
  y += 5;
